...
class Qwen3Backbone(torch.nn.Module):
    def __init__(
        ...
        # 本地 Cosmos 副本优先(GR00T_COSMOS_PATH), 避免 gated 远程下载
        _cosmos_local = os.environ.get("GR00T_COSMOS_PATH")
        if _cosmos_local and model_name == "nvidia/Cosmos-Reason2-2B":
            model_name = _cosmos_local
        try:
