...
用法:
  ...
  python pi05_oellm_convert_bcGemmaExpert.py \
      --convert-bc-path /home/chao.wu/pi_oellm/ROBOTREA/M7_pickplace_example_ckpt_hbm1/gemma_expert/pi05_gemma_expert_pi05_action_horizon_20_ptq.convert.bc \
      --output-hbm-path /home/chao.wu/pi_oellm/ROBOTREA/M7_pickplace_example_ckpt_hbm1/gemma_expert/pi05_gemma_expert_pi05_action_horizon_20_ptq.hbm \
      --march nash-p \
      --jobs 32
  ...
  cd /home/chao.wu/pi_oellm/ROBOTREA/robotera_vla_drobotics/scripts_drobotics/oellm_cauchy
  python pi05_oellm_convert_bcGemmaExpert.py \
      --convert-bc-path /home/chao.wu/pi_oellm/ROBOTREA/M7_pickplace_example_ckpt_hbm1/gemma_expert_O0/pi05_gemma_expert_pi05_action_horizon_20_ptq.convert.bc \
      --output-hbm-path /home/chao.wu/pi_oellm/ROBOTREA/M7_pickplace_example_ckpt_hbm1/gemma_expert_O0/pi05_gemma_expert_pi05_action_horizon_20_ptq.hbm \
      --march nash-p \
      --jobs 32 \
      --opt 0
"""
...
def compile_expert_bc_to_hbm(
    convert_bc_path: str,
    output_hbm_path: str,
    compile_kwargs: dict,
):
    """从 .convert.bc 编译 Gemma Expert 到 HBM，并运行 perf。

    流程与 model_gemma_expert.py 的 compile() 中
    ...
    # --- Step 2: 编译到 HBO ---
    # Gemma Expert 特有参数（与 model_gemma_expert.py compile() 一致）
    ...
    print("\n" + "=" * 60)
    print("Gemma Expert bc→HBM compilation complete:")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Pi05 Gemma Expert bc→HBM 编译工具",
    ...
    p.add_argument(
        ...
        help="Path to the .convert.bc file (e.g. pi05_gemma_expert_pi05_action_horizon_20_ptq.convert.bc).",
    )
    p.add_argument(
        "--output-hbm-path", type=str, required=True,
        help="Path for the output .hbm file (e.g. pi05_gemma_expert_pi05_action_horizon_20_ptq.hbm).",
    )

    # 编译参数（与 pi05_oellm_convert.py 保持一致）
    p.add_argument(
...
def main():
    ...
    # 构建编译参数（与 pi05_oellm_convert.py 中 expert_kwargs 基础参数一致）
    compile_kwargs = {
        "march": args.march,
        "jobs": args.jobs,
        "progress_bar": True,
    ...
    print("=" * 60)
    print("Pi05 Gemma Expert bc→HBM Converter")
    ...
    print(f"  enable_hpc:   True")
    print("=" * 60)

    compile_expert_bc_to_hbm(
