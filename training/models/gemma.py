...
(这个模块是从 big_vision 中借用的 Gemma 适配器，用于 Pi 模型。)
...
We follow this einsum axis naming convention:
  ...
  G: num query heads per k/v head (每个 key/value head 对应的 query head 数量，不必每个 query head 都有独立的 key/value)
    场景1: 计算优化,最终会 concat 回d_model维度
    场景2: 语义上独立的 query,用于不同任务或未来步骤

  H: head dim (每个 attention head 的维度)
  D: d_model ("features")  D = N * H
"""
...
import training.models.lora as lora
...
def get_config(variant: Variant) -> Config:
    ...
    if variant == "gemma_300m":
        ...
        return Config(
            width=1024,
            depth=18,  # 18个Transformer block (每一个block包含一个Attention和一个FeedForward)
            mlp_dim=4096, # Self-Attention之后的FeedForward隐藏层维度
            num_heads=8,
            num_kv_heads=1,  # Key/Value 的头数（使用 Grouped Query Attention，KV 共享）
            head_dim=256,
        )
    if variant == "gemma_2b":
        return Config(
...
# l: num layers
# b: batch
# _t: 时间步——表示缓存中当前已经存在的 token 序列长度
#     假设当前在第5个时间步，要预测第6个token时，模型只需计算新 token 的 K/V，并拼接在缓存后面 _t = 6
# _k: num kv heads
# _h: head dim
...
class Module(nn.Module):
    ...
    def setup(self):
        ...
        self.layers = nn.scan(  # self.layers 是一个 通过 nn.scan 包装的 Block
            block_cls,
