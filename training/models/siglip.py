...
class Encoder1DBlock(nn.Module):
    ...
    def __call__(self, x, deterministic=True):  # noqa: FBT002
        out = {}
        # jax.debug.print("Layer input shape: {}", x.shape)
...
class _Module(nn.Module):
    ...
    def __call__(self, image, *, train=False):
        ...
        image = jnp.asarray(image, jnp.float32)    # (1, 224, 224, 3)
        # Patch extraction
        x = out["stem"] = nn.Conv(
            self.width,
            self.patch_size,
        ...
        n, _, c = x.shape  # n,l,d   # (1, 256, 1152)
        x = nn.Dropout(rate=self.dropout)(x, not train) #train=False
        ...
        x_2d = jnp.reshape(encoded, [n, h, w, -1])  # (1, 16, 16, 1152)

        if self.rep_size:  #False
        ...
        out["pre_logits_2d"] = x_2d # (1, 16, 16, 1152)
        out["pre_logits"] = x # (1, 256,1152)

        if self.num_classes:
            kw = {"kernel_init": nn.initializers.zeros} if self.head_zeroinit else {}
            head = nn.Dense(self.num_classes, dtype=self.dtype_mm, name="head", **kw)
            x_2d = out["logits_2d"] = head(x_2d) #(1, 16,16, 2048)
            x = out["logits"] = head(x) # (1, 256, 2048)
